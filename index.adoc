...
`{nbsp}{nbsp}{nbsp}{nbsp}commandId: Int? = null` +
`) -> Self`
| Since          | 1.1
| Description
| Prepares the writing of data to the system block of an ST25/SRT512 storage card. This operation is specific to
  ...
  *Important:* the application is responsible for ensuring that the write operations are coherent with the target
  ...
  the transaction fails if the physical state does not match the provided data. When a `commandId` is supplied, it
  MAY later be obtained via the `commandId` property of the error, if this specific command causes one to be raised.
| Parameters
| `data`      -- the data to be written to the system block (expected final state). +
  `commandId` -- the application-supplied identifier of this command, or `null` if none.
| Returns        | The current instance (`Self`).
...
| Prepares the writing of one or more blocks starting from the specified block offset. The number of blocks
  ...
  *Important:* the application is responsible for ensuring that the write operations are coherent with the target
  ...
  transaction fails if the physical state does not match the provided data. When a `commandId` is supplied, it MAY
  later be obtained via the `commandId` property of the error, if this specific command causes one to be raised.
| Parameters
