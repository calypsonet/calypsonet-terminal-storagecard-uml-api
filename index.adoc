...
* the following storage card products, all contactless cards compliant with <<ref_iso14443,ISO/IEC 14443>> and
  identified by the values of <<type_StorageCardProductType,`StorageCardProductType`>>:
** MIFARE Ultralight,
...
| Transaction manager for storage cards and its security settings.
|===
...
| Returns the data block at the specified block address, as known in the memory image of the storage card.
  `null` if the block has not been read.
| Parameters     | `blockAddress` -- the address of the block to retrieve.
| Returns        | The non-empty block data, or `null` if the block has not been read.
...
| Returns the data blocks within the specified range from the memory image of the storage card. The returned
  array contains the blocks in order, from `fromBlockAddress` to `toBlockAddress` (both inclusive). `null` if
  at least one block of the range has not been read.
| Parameters
| `fromBlockAddress` -- the starting block address (inclusive). +
  `toBlockAddress`   -- the ending block address (inclusive).
| Returns        | The non-empty data blocks in the specified range, or `null` if at least one of them has not been read.
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature      | `getProductType() -> StorageCardProductType`
| Since          | 1.0
| Description    | Returns the product type of the storage card.
| Returns        | A <<type_StorageCardProductType,`StorageCardProductType`>>.
| Pre-conditions | None.
| Errors         | None.
|===
...
`{nbsp}{nbsp}{nbsp}{nbsp}productType: StorageCardProductType` +
...
| Parameters     | `productType` (<<type_StorageCardProductType,`StorageCardProductType`>>) -- the targeted storage card product type.
...
`{nbsp}{nbsp}{nbsp}{nbsp}securitySettings: StorageCardSecuritySettings` +
`) -> StorageCardTransactionManager`
| Since          | 1.0
| Description    | Returns a new <<type_StorageCardTransactionManager,`StorageCardTransactionManager`>> bound to the supplied reader, to the initial card data provided by the selection process and to the supplied security settings. A default <<type_StorageCardSecuritySettings,`StorageCardSecuritySettings`>> disables every duration bound.
| Parameters
| `reader` (`CardReader` (<<ref_cna_tr,CNA-TR-API>>)) -- the reader through which the card is reached. +
  ...
  `securitySettings` (<<type_StorageCardSecuritySettings,`StorageCardSecuritySettings`>>) -- the security settings to be used.
| Returns        | A <<type_StorageCardTransactionManager,`StorageCardTransactionManager`>>.
| Pre-conditions | *Argument* -- every parameter is non-`null`.
| Errors         | None.
|===

[[type_StorageCardSecuritySettings]]
==== Storage Card Security Settings

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `StorageCardSecuritySettings`
| Kind       | Data
| Namespace  | `storagecard.transaction`
| Since      | 2.0
| Purpose    | Carries the security settings of a storage card transaction, built directly by the application. A single instance MAY be shared by every transaction of a terminal, whatever the product type of the card presented.
|===
...
| `readCommandMaxDurations`
| `Map<StorageCardProductType, Long>`
| `emptyMap()`
| 2.0
| Maximum duration, in microseconds, of the exchange of a read command, for each product type. The bound
  applies to *each* read command prepared on the <<type_StorageCardTransactionManager,`StorageCardTransactionManager`>>,
  taken individually. The bound declared for the product type of the card bound to the transaction manager
  applies; a product type absent from the map is not bounded. Every declared duration is strictly positive.

  The effective resolution of the measurement depends on the implementation, which limits the detection to the
  terminals whose measurement chain is fine enough.

  This bound is a countermeasure against *card emulation*: a generic RFID emulation device does not answer a
  read command in the same time as the chip of a genuine storage card, so an abnormally long exchange reveals
  that the answer does not come from the expected product. The bound does not apply to the card selection, nor
  to the write and authentication commands.
|===
...
[[op_StorageCardSelectionExtension_prepareMifareClassicAuthenticateWithKey]]
...
| Prepares a MIFARE Classic authentication command using the supplied 6-byte key. This operation is specific
  ...
  way, it is forwarded to the reader to be stored as a *volatile* key at index 0 (see <<ref_pcsc,PC/SC>>
  *Load Key* command). The volatile key is temporary and is erased after usage, when the reader is powered
  ...
  <<op_StorageCardSelectionExtension_prepareMifareClassicAuthenticateWithKeyNumber,`prepareMifareClassicAuthenticateWithKeyNumber`>>
...
[[op_StorageCardSelectionExtension_prepareMifareClassicAuthenticateWithKeyNumber]]
...
| Since          | 1.1
| Description
| Prepares a MIFARE Classic authentication command using a key referenced by its storage index in the reader's
  key storage. This operation is specific to MIFARE Classic cards and MUST be prepared before reading from or
  writing to protected sectors. The authentication applies to the entire sector containing the supplied block
  address. Referencing the key by index allows pre-configured keys to be used without transmitting their value
  over the communication channel, which provides enhanced security. Once authenticated, subsequent read and
  write operations within the same sector MAY be performed without re-authentication, until the card is
  removed from the field or another sector is accessed.
| Parameters
| `blockAddress`         -- the address of any block within the sector to authenticate. +
  `mifareClassicKeyType` (<<type_MifareClassicKeyType,`MifareClassicKeyType`>>) -- the type of key to use (Key A or Key B). +
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature      | `prepareReadBlock(blockAddress: Int) -> Self`
| Since          | 1.0
| Description
| Prepares the reading of a specific block. Block addresses start at `0`; the maximum value is
  <<type_StorageCardProductType,`StorageCardProductType.blockCount`>> - 1. Once processed, the result is
  available in the <<type_StorageCard,`StorageCard`>> via <<op_StorageCard_getBlock,`getBlock`>>.
| Parameters     | `blockAddress` -- the address of the block to be read.
| Returns        | The current instance (`Self`).
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
  *Range* -- the block address is within range.
| Errors         | None.
| See also       | <<type_StorageCardProductType,`StorageCardProductType.blockCount`>>
|===
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature      | `prepareSt25ReadSystemBlock() -> Self`
| Since          | 1.2
| Description
...
[[op_StorageCardTransactionManager_prepareMifareClassicAuthenticateWithKey]]
===== Prepare MIFARE Classic Authenticate With Key
...
| Prepares a MIFARE Classic authentication command using the supplied 6-byte key. This operation is specific
  ...
  *Load Key* command). The volatile key is temporary and is erased after usage, when the reader is powered
  off. *Security note:* this operation transmits the key value over the application-reader communication
  channel. For production environments and security-sensitive applications, it is RECOMMENDED to use the
  <<op_StorageCardTransactionManager_prepareMifareClassicAuthenticateWithKeyNumber,`prepareMifareClassicAuthenticateWithKeyNumber`>>
  instead, which references a key already stored in the reader without transmitting its value.
| Parameters
| `blockAddress`         -- the address of any block within the sector to authenticate. +
  `mifareClassicKeyType` (<<type_MifareClassicKeyType,`MifareClassicKeyType`>>) -- the type of key to use (Key A or Key B). +
...
[[op_StorageCardTransactionManager_prepareMifareClassicAuthenticateWithKeyNumber]]
===== Prepare MIFARE Classic Authenticate With Key Number
...
| Prepares the reading of a specific block. Block addresses start at `0`; the maximum value is
  ...
  When the transaction manager has been created with
  <<type_StorageCardSecuritySettings,`StorageCardSecuritySettings`>> declaring a bound for the product type of
  the card, each read command exchange is bounded by that duration; an overrun raises
  <<type_StorageCardInvalidCardResponse,`StorageCardInvalidCardResponse`>>.
| Parameters     | `blockAddress` -- the address of the block to be read.
| Returns        | The current instance (`Self`).
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
  *Range* -- the block address is within range.
...
| Prepares the writing of data to the system block of an ST25/SRT512 storage card. This operation is specific to
  ...
  <<type_StorageCardProductType,`StorageCardProductType`>>.
  ...
  *Important:* the application is responsible for ensuring that the write operations are coherent with the target
  ...
  the transaction fails if the physical state does not match the provided data. The `commandId` MAY later be
  obtained via the `commandId` property of the error, if this specific command causes one to be raised.
| Parameters
...
| Errors         | None.
| See also       | <<type_StorageCardProductType,`StorageCardProductType.blockSize`>>
|===
...
| Prepares the writing of one or more blocks starting from the specified block offset. The number of blocks
  ...
  *Important:* the application is responsible for ensuring that the write operations are coherent with the target
  ...
  transaction fails if the physical state does not match the provided data. The `commandId` MAY later be obtained
  via the `commandId` property of the error, if this specific command causes one to be raised.
| Parameters
| `commandId`        -- the application-supplied identifier of this command. +
...
[[type_StorageCardProductType]]
==== Storage Card Product Type

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `StorageCardProductType`
| Kind       | Enumeration
| Namespace  | `storagecard.card`
| Since      | 1.0
...
| `Int`
| 1.0
| Number of blocks in the main memory area of the card. Additional *system* blocks MAY exist and are not
...
| Indicates whether the product type exposes an accessible system block. For ST25/SRT512 cards, the system
  ...
  `prepare*` operations during selection or transaction processing.

| `hasWriteAcknowledgment`
| `Boolean`
| 1.0
| Indicates whether the product type provides a reliable acknowledgment after write operations: when `true`, a
  successful write response guarantees that the data has been correctly stored; when `false`, a verification
  read is performed to confirm the write.

| `hasAuthentication`
| `Boolean`
| 1.1
| Indicates whether the product type requires authentication before read/write operations: when `true`, the
  application MUST authenticate the targeted sector before issuing read or write commands.
|===

[cols="^.^3,^.^1,^.^1,^.^1,^.^1,^.^1,^.^1,.^3", options="header"]
|===
| Value               | `blockCount` | `blockSize` | `hasSystemBlock` | `hasWriteAcknowledgment` | `hasAuthentication` | Since | Description
| `MIFARE_ULTRALIGHT` | `16`         | `4`         | `false`          | `true`                   | `false`             | 1.0   | MIFARE Ultralight.
| `MIFARE_CLASSIC_1K` | `64`         | `16`        | `false`          | `true`                   | `true`              | 1.1   | MIFARE Classic 1K.
| `MIFARE_CLASSIC_4K` | `256`        | `16`        | `false`          | `true`                   | `true`              | 1.1   | MIFARE Classic 4K.
| `ST25_SRT512`       | `16`         | `4`         | `true`           | `false`                  | `false`             | 1.0   | ST25 SRT512.
|===
...
The storage-card errors -- <<type_StorageCardReaderCommunication,`StorageCardReaderCommunication`>>,
<<type_StorageCardCardCommunication,`StorageCardCardCommunication`>>, <<type_StorageCardInvalidCardResponse,`StorageCardInvalidCardResponse`>>
and <<type_StorageCardAuthenticationFailed,`StorageCardAuthenticationFailed`>> -- additionally carry `blockAddress` (`Int?`),
...
<<type_StorageCardInvalidCardResponse,`StorageCardInvalidCardResponse`>> is also raised when the duration of a read command
exchange exceeds the bound declared for the product type of the card in
<<type_StorageCardSecuritySettings,`StorageCardSecuritySettings`>>.

[[type_StorageCardAuthenticationFailed]]
==== Storage Card Authentication Failed

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `StorageCardAuthenticationFailed`
| Kind       | Error
| Namespace  | `storagecard`
| Since      | 1.1
...
[[type_StorageCardCardCommunication]]
==== Storage Card Card Communication

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `StorageCardCardCommunication`
| Kind       | Error
| Namespace  | `storagecard`
...
[[type_StorageCardInvalidCardResponse]]
==== Storage Card Invalid Card Response

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `StorageCardInvalidCardResponse`
| Kind       | Error
| Namespace  | `storagecard`
...
[[type_StorageCardReaderCommunication]]
==== Storage Card Reader Communication

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `StorageCardReaderCommunication`
| Kind       | Error
| Namespace  | `storagecard`
