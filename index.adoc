...
| Prepares the writing of data to the system block of an ST25/SRT512 storage card, and attaches an
  ...
  the `idCommand` property of the error, if this specific command causes one to be raised.
| Parameters
...
The storage-card errors -- <<type_SCReaderCommunication,`SCReaderCommunication`>>,
<<type_SCCardCommunication,`SCCardCommunication`>>, <<type_SCInvalidCardResponse,`SCInvalidCardResponse`>>
and <<type_SCAuthenticationFailed,`SCAuthenticationFailed`>> -- additionally carry `blockAddress` (`Int?`),
the block address involved in the error, and `idCommand` (`Int?`), the application-supplied identifier of
